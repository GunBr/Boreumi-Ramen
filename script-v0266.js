(() => {
  ...
  document.title = "보름이의 라면포차 V0.26.6";
  ...
  function showNextGuestDialogue() {
    ...
    const positionDialogue = () => {
      ...
      if (activeGuest) {
        ...
        const guestBox = {
          left: guestRow.offsetLeft + (rowMatrix?.e || 0) + slot.offsetLeft + guestArt.offsetLeft,
          top: guestRow.offsetTop + (rowMatrix?.f || 0) + slot.offsetTop + guestArt.offsetTop,
        ...
        const left = Math.max(whisperHalfWidth, Math.min(stage.offsetWidth - whisperHalfWidth, guestBox.left + guestBox.width / 2));
  ...
  async function browserQA() {
    ...
    try {
      const [manifestResponse, cssResponse, experienceResponse, mobileResponse, storyResponse, bootResponse, workerResponse, patchResponse, patchV0262Response, patchV0263Response, patchV0264Response, patchV0265Response, patchV0266Response] = await Promise.all([
    ...
    result.parallelCriticalLoading = bootSource.includes("preloadCriticalAssets")
      ...
      && bootSource.includes('version: "0.26.6"');
    ...
    result.saveRecoveryReady = !!$("#exportSaveButton")
      ...
      && exportProbe.gameVersion === "0.26.6";
    ...
    result.guestCounterReachesBothEdges = Math.abs(guestCounterRect.left - guestStageRect.left) <= 2
      ...
      && Math.abs(parseFloat(guestCounterStyle.top) + parseFloat(guestCounterStyle.height) - 638) <= .5;
    result.guestGroupLoweredTogether = parseFloat(getComputedStyle($("#guestRow")).top) === 172
      && parseFloat(getComputedStyle($("#ppomiPerch")).top) === 329
      && parseFloat(guestCounterStyle.top) === 448;
