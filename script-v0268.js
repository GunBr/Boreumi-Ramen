(() => {
  ...
  document.title = "보름이의 라면포차 V0.26.8";
  ...
  const State = {
    ...
    takeoutTimers: [],
    boreumiTimer: null,
    ppomi: { x: 0, direction: 1, ready: false, lastGuestKey: "" },
    waste: 0,
    served: 0,
    missed: 0,
    takeoutServed: 0,
  ...
  function build() {
    ...
    const row = $("#guestRow");
    Guests.forEach(guest => {
      row.insertAdjacentHTML("beforeend", `<article class="guest-slot" data-guest="${guest.index}"><div class="bubble" aria-label="주문"><div class="order-items"></div><span class="satisfaction" aria-hidden="true"></span></div><div class="guest-seat" role="img" aria-label="빈 의자"></div><div class="guest-art" role="img" aria-label="방문 손님"></div><div class="eating-scene" aria-hidden="true"><i></i><b></b></div><div class="patience" aria-label="손님 인내심"><i></i></div></article>`);
    });
  ...
  function showNextGuestDialogue() {
    ...
    const positionDialogue = () => {
      ...
      if (activeGuest) {
        ...
        const top = Math.max(360, Math.min(555, guestBox.top + guestBox.height + 12));
  ...
  function renderGuest(guest) {
    ...
    const eatingTimer = slot.querySelector(".eating-scene b");
    if (eatingTimer) eatingTimer.textContent = guest.eating ? `${Math.max(1, Math.ceil(guest.eatingRemaining / 1000))}초` : "";
  ...
  function completeTakeout(order) {
    ...
    const resetTimer = setTimeout(() => resetTakeoutOrder(order, false), 820);
    State.takeoutTimers.push(resetTimer);
  ...
  function tickGuests() {
    ...
    Guests.forEach(guest => {
      ...
      const previousSecond = Math.ceil(guest.eatingRemaining / 1000);
      guest.eatingRemaining = Math.max(0, guest.eatingRemaining - elapsed);
      if (guest.eatingRemaining <= 0) dismissGuest(guest.index);
      else if (Math.ceil(guest.eatingRemaining / 1000) !== previousSecond) renderGuest(guest);
    });
    if (!Config.guests.waitsForever) {
      Guests.forEach(guest => {
        if (!guest.active || guest.serving) return;
  ...
  function tickPpomi(elapsed) {
    ...
    const visibleSlots = Guests.slice(0, guestCapacityForLevel()).map(guest => $(`[data-guest="${guest.index}"]`)).filter(slot => slot && !slot.hidden);
    if (!visibleSlots.length) return;
    const centers = visibleSlots.map(slot => {
      const box = stageBoxFor(slot);
      return box.left + box.width / 2;
    });
    const leftBound = Math.min(...centers) - 54;
    const rightBound = Math.max(...centers) + 54;
    ...
    let nextX = previousX + movementDirection * elapsed * .072;
    if (nextX >= rightBound) {
      nextX = rightBound;
      State.ppomi.direction = -1;
    ...
    State.ppomi.x = nextX;
    const ppomiLeft = `${nextX - (perch.offsetWidth || 178) / 2}px`;
    perch.style.left = ppomiLeft;
    perch.style.setProperty("--ppomi-left", ppomiLeft);
  ...
  async function browserQA() {
    ...
    try {
      const [manifestResponse, cssResponse, experienceResponse, mobileResponse, storyResponse, bootResponse, workerResponse, patchResponse, patchV0262Response, patchV0263Response, patchV0264Response, patchV0265Response, patchV0266Response, patchV0267Response] = await Promise.all([
    ...
    result.parallelCriticalLoading = bootSource.includes("preloadCriticalAssets")
      ...
      && bootSource.includes('version: "0.26.7"');
    ...
    result.guestLeavesAfterServe = Guests[0].active
      ...
      && Boolean($(`[data-guest="0"] .eating-scene i`));
