(() => {
  ...
  document.title = "보름이의 라면포차 V0.26.9";
  ...
  const Config = {
    ...
    guests: { tickMs: 100, waitsForever: true, patienceMs: 40000, wrongPenaltyMs: 0, eatingMs: 10000, nextArrivalMs: 5000 },
    takeout: {
      waitsForever: true,
      patienceMs: 36000,
      missedPenalty: 500,
    ...
    firstArrivals: [700, 3100, 5500, 7900, 10300]
  };
  ...
  const FoodArt = {
    ...
    potKimchi: "assets/art-v0269/food-ramen-kimchi-red-v1.webp",
    potCheese: "assets/art-v025/food-ramen-cheese-v1.webp",
    potEggScallion: "assets/art-v026/food-ramen-egg-scallion-v1.webp",
    potKimchiEgg: "assets/art-v0269/food-ramen-kimchi-egg-red-v1.webp",
    potCheeseEgg: "assets/art-v026/food-ramen-cheese-egg-v1.webp",
    potKimchiCheese: "assets/art-v0269/food-ramen-kimchi-cheese-red-v1.webp",
  ...
  const MenuCatalog = Object.freeze({
    ...
    soju: Object.freeze({ id: "soju", kind: "drink", label: "소주", art: "assets/art-v012/drink-soju-v1.webp", price: 2500 }),
    beer: Object.freeze({ id: "beer", kind: "drink", label: "맥주", art: "assets/art-v012/drink-beer-v1.webp", price: 3000 }),
    somaek: Object.freeze({ id: "somaek", kind: "drink", label: "소맥", art: "assets/art-v012/drink-somaek-v1.webp", price: 3500 }),
    makgeolli: Object.freeze({ id: "makgeolli", kind: "drink", label: "막걸리", art: "assets/art-v012/drink-makgeolli-v1.webp", price: 3000 })
  });
  ...
  const ProgressionMilestones = Object.freeze([
    ...
    Object.freeze({ day: 50, stall: 2, seats: 5, customers: 10, label: "포차 LV.2 + DAY 50" }),
    Object.freeze({ day: 1, stall: 3, seats: 5, customers: 12, label: "포차 LV.3" }),
    Object.freeze({ day: 1, stall: 4, seats: 5, customers: 15, label: "포차 LV.4" }),
    Object.freeze({ day: 1, stall: 5, seats: 5, customers: 18, label: "포차 LV.5" })
  ]);
  ...
  const StationUpgradeCatalog = Object.freeze({
    pot: Object.freeze({
      ...
      title: "라면 화구",
      subtitle: "모든 냄비 강화",
      costs: Object.freeze([18000, 48000, 110000, 240000]),
    ...
    grill: Object.freeze({
      ...
      title: "만두 그릴",
      subtitle: "모든 그릴 강화",
      costs: Object.freeze([16000, 42000, 95000, 210000]),
    ...
    oden: Object.freeze({
      ...
      title: "오뎅바",
      subtitle: "보온·회전율 강화",
      costs: Object.freeze([14000, 36000, 82000, 180000]),
  ...
  const StallUpgradeCatalog = Object.freeze({
    maxLevel: 5,
    costs: Object.freeze([90000, 250000, 600000, 1400000]),
    benefits: Object.freeze({
      2: "냄비 3개·포장 주문 1건 해금 · DAY 50부터 최대 좌석 5석/손님 10명",
      3: "그릴 2개·완성대 2칸 해금 · 좌석 5석/손님 12명",
      4: "포장 주문 2건·완성대 3칸 · 좌석 5석/손님 15명",
      5: "포장 주문 3건·완성대 4칸 · 좌석 5석/손님 18명"
    })
  });
  ...
  function applyStallLevel() {
    ...
    Guests?.forEach(guest => {
      ...
      const meal = $(`[data-guest-meal="${guest.index}"]`);
      if (meal && locked) {
        meal.hidden = true;
        meal.setAttribute("aria-hidden", "true");
      }
      if (locked && guest.active) {
        guest.active = false;
        guest.serving = false;
        guest.eating = false;
        guest.eatingRemaining = 0;
        guest.eatingKind = "";
        guest.order = null;
  ...
  const State = {
    ...
    takeoutTimers: [],
    boreumiTimer: null,
    ppomi: { x: 0, direction: 1, ready: false, lastGuestKey: "", centers: [], leftBound: 0, rightBound: 0, spriteWidth: 178 },
    waste: 0,
    served: 0,
    missed: 0,
    takeoutServed: 0,
  ...
  const Tutorial = {
    ...
    steps: Object.freeze({
      ...
      serveDrink: Object.freeze({ order: 6, eyebrow: "뽀미 서빙 · 6/7", title: "주류는 뽀미가 도와줘요", text: "뽀미가 손님 앞을 지나면 주문한 술을 자동으로 전달해요. 오른쪽 주류 선반에서 재고만 확인하면 돼요." }),
  ...
  function renderDrinkStockRail() {
    const rail = $("#drinkStockRail");
    if (!rail) return;
    const drinks = InventoryCategories.find(entry => entry.id === "drinks")?.items || [];
    rail.innerHTML = drinks.map(item => {
      const unlocked = isIngredientUnlocked(item.id);
      const stock = ingredientStock(item.id);
      const status = unlocked ? `${stock}개` : `LV.${item.unlockLevel}`;
      return `<span class="drink-stock${unlocked ? "" : " locked"}${stock <= 0 ? " sold-out" : ""}" data-drink-stock="${item.id}" aria-label="${item.label} 재고 ${status}"><img src="${item.art}" alt=""><small>${item.label}</small><b>${status}</b></span>`;
    }).join("");
  }

  function renderDockCategory(categoryId) {
    if (categoryId === "drinks") return renderDrinkStockRail();
  ...
  function buildDock() {
    ...
    InventoryCategories.filter(category => category.id !== "drinks").forEach(category => {
  ...
  function build() {
    ...
    const row = $("#guestRow");
    const mealLayer = $("#guestMealLayer");
    Guests.forEach(guest => {
      ...
      mealLayer?.insertAdjacentHTML("beforeend", `<article class="guest-meal" data-guest-meal="${guest.index}" aria-hidden="true" hidden></article>`);
    });
  ...
  function burstAt(element, kind = "drop", count = 8) {
    ...
    const busyScene = document.documentElement.dataset.mobileLayout === "true"
      || Guests.filter(guest => guest.active).length >= 4
      || layer.childElementCount >= 18;
    const particleCount = busyScene ? Math.min(count, 5) : Math.min(count, 12);
    for (let index = 0; index < particleCount; index += 1) {
      const particle = document.createElement("i");
      const angle = Math.PI * 2 * index / particleCount + Math.random() * .28;
  ...
  function spriteFor(appliance) {
    ...
    if (appliance.type === "grill" && appliance.recipeId === "grilled_dumpling" && ["cooking", "ready"].includes(appliance.state)) {
      if (appliance.servingsShown <= 1) return "cooking-dumpling-one";
      if (appliance.servingsShown === 2) return "cooking-dumpling-two";
      return "cooking-dumpling";
    }
  ...
  function renderAppliance(appliance) {
    ...
    element.dataset.servingCount = isBatchAppliance(appliance) && appliance.state !== "empty" ? String(Math.max(1, appliance.servingsShown || 1)) : "0";
    const odenStatus = appliance.type === "oden" && appliance.state === "ready" ? ` · 계속 서빙 가능 · 오뎅 ${Math.max(1, appliance.servingsShown)}개 보임` : "";
    const dumplingStatus = appliance.type === "grill" && appliance.recipeId === "grilled_dumpling" && appliance.state === "ready" ? ` · 계속 서빙 가능 · 군만두 ${Math.max(1, appliance.servingsShown)}개 보임` : "";
    element.setAttribute("aria-label", `${appliance.type === "pot" ? `냄비 ${appliance.slot + 1}` : appliance.type === "grill" ? `그릴 ${appliance.slot + 1}` : "오뎅바"} · ${applianceStateLabel(appliance)}${odenStatus}${dumplingStatus}`);
  ...
  function showNextGuestDialogue() {
    ...
    const positionDialogue = () => {
      ...
      if (activeGuest) {
        ...
        const stage = $("#stage");
        /* Position from the guest slot: rotated iOS swaps visual rectangle axes and an
           absolutely positioned portrait can skip the static grid slot as offsetParent. */
        const guestRow = $("#guestRow");
        const rowTransform = getComputedStyle(guestRow).transform;
        const rowMatrix = rowTransform && rowTransform !== "none" ? new DOMMatrixReadOnly(rowTransform) : null;
        const artTransform = getComputedStyle(guestArt).transform;
        const artMatrix = artTransform && artTransform !== "none" ? new DOMMatrixReadOnly(artTransform) : null;
        const guestBox = {
          left: guestRow.offsetLeft + (rowMatrix?.e || 0) + slot.offsetLeft + guestArt.offsetLeft + (artMatrix?.e || 0),
          top: guestRow.offsetTop + (rowMatrix?.f || 0) + slot.offsetTop + guestArt.offsetTop + (artMatrix?.f || 0),
          width: guestArt.offsetWidth,
          height: guestArt.offsetHeight
        };
        const whisperHalfWidth = document.documentElement.dataset.mobileLayout === "true" ? 236 : 215;
        const guestCenter = guestBox.left + guestBox.width / 2;
        const left = Math.max(whisperHalfWidth, Math.min(stage.offsetWidth - whisperHalfWidth, guestCenter));
        const pointerX = Math.max(30, Math.min(whisperHalfWidth * 2 - 30, whisperHalfWidth + guestCenter - left));
        const top = Math.max(270, Math.min(432, guestBox.top + guestBox.height - 94));
        element.style.setProperty("--whisper-x", left + "px");
        element.style.setProperty("--whisper-y", top + "px");
        element.style.setProperty("--whisper-tip-x", pointerX + "px");
      } else {
        element.style.setProperty("--whisper-x", "50%");
        element.style.setProperty("--whisper-y", "352px");
        element.style.setProperty("--whisper-tip-x", "50%");
      }
    };
    positionDialogue();
  ...
  function eatingKindForGuest(guest) {
    const foodId = guest.order?.items.find(item => MenuCatalog[item.id]?.kind === "food")?.id || "";
    if (foodId === "warm_oden") return "oden";
    if (foodId === "grilled_dumpling") return "dumpling";
    return foodId.startsWith("ramen_") ? "ramen" : "";
  }
  ...
  function renderGuestMeal(guest) {
    const meal = $(`[data-guest-meal="${guest.index}"]`);
    if (!meal) return;
    const items = guest.order?.items || [];
    const visible = guest.active && guest.eating && items.length > 0;
    meal.hidden = !visible;
    meal.setAttribute("aria-hidden", String(!visible));
    if (!visible) {
      meal.replaceChildren();
      meal.removeAttribute("aria-label");
      return;
    }
    meal.innerHTML = items.map(orderItem => {
      const menuItem = MenuCatalog[orderItem.id];
      return `<span class="guest-meal-item ${menuItem.kind}" title="${menuItem.label}"><img src="${menuItem.art}" alt=""></span>`;
    }).join("");
    meal.setAttribute("aria-label", `${CustomerById[guest.customerId]?.name || "손님"} 앞에 놓인 ${items.map(item => MenuCatalog[item.id].label).join(", ")}`);
  }
  ...
  function renderGuest(guest) {
    ...
    slot.classList.toggle("eating", guest.eating);
    slot.dataset.eating = guest.eatingKind || "";
  ...
  function managedTimer(collection, callback, delay) {
    const timer = setTimeout(() => {
      const index = collection.indexOf(timer);
  ...
  function scheduleTakeout(index, delay) {
    ...
    managedTimer(State.takeoutTimers, () => activateTakeout(index), delay);
  }
  ...
  function expireTakeout(order) {
    ...
    managedTimer(State.takeoutTimers, () => resetTakeoutOrder(order, true), 760);
  }
  ...
  function completeTakeout(order) {
    ...
    managedTimer(State.takeoutTimers, () => resetTakeoutOrder(order, false), 820);
    scheduleTakeout(order.index, Config.takeout.repeatDelayMs);
  }
  ...
  function scheduleGuest(index, delay) {
    ...
    managedTimer(State.guestTimers, () => activateGuest(index), delay);
  }
  ...
  function dismissGuest(index) {
    ...
    guest.customerId = null;
    renderGuest(guest);
    if (State.running && !State.closing && index < guestCapacityForLevel()) scheduleGuest(index, Config.guests.nextArrivalMs);
    checkClosingComplete();
  }
  ...
  function expireGuest(guest) {
    ...
    managedTimer(State.guestTimers, () => dismissGuest(guest.index), 720);
  }
  ...
  function tickGuests() {
    ...
    if (State.tutorialMode) return;
    Guests.forEach(guest => {
      if (!guest.active || !guest.eating) return;
      guest.eatingRemaining = Math.max(0, guest.eatingRemaining - elapsed);
      if (guest.eatingRemaining <= 0) dismissGuest(guest.index);
    });
    if (!Config.guests.waitsForever) {
      Guests.forEach(guest => {
        if (!guest.active || guest.serving) return;
  ...
  function stageBoxFor(element) {
    ...
    const addNode = node => {
      if (!node || node === stage || counted.has(node)) return;
      counted.add(node);
  ...
  function refreshGuestStageLayout() {
    if (!State.ppomi || !$("#guestRow")) return;
    const centers = Guests.map(guest => {
      const slot = $(`[data-guest="${guest.index}"]`);
      if (!slot || slot.hidden) return null;
      const box = stageBoxFor(slot);
      const center = box.left + box.width / 2;
      const meal = $(`[data-guest-meal="${guest.index}"]`);
      if (meal) meal.style.setProperty("--meal-x", `${center}px`);
      return center;
    });
    const visibleCenters = centers.filter(Number.isFinite);
    State.ppomi.centers = centers;
    State.ppomi.leftBound = visibleCenters.length ? Math.min(...visibleCenters) - 54 : 0;
    State.ppomi.rightBound = visibleCenters.length ? Math.max(...visibleCenters) + 54 : 0;
    State.ppomi.ready = false;
  }
  ...
  function resetPpomi() {
    const perch = $("#ppomiPerch");
    if (!perch) return;
    State.ppomi.x = 0;
    State.ppomi.direction = 1;
    State.ppomi.ready = false;
    State.ppomi.lastGuestKey = "";
    perch.classList.remove("walking", "serving-drink", "pose-wave", "pose-groom");
    perch.classList.add("pose-sleep");
    perch.style.removeProperty("left");
    perch.style.removeProperty("--ppomi-left");
    perch.style.removeProperty("--ppomi-x");
    perch.removeAttribute("data-direction");
  }
  ...
  function startPpomi() {
    ...
    State.ppomi.ready = false;
    State.ppomi.lastGuestKey = "";
    if (!State.ppomi.centers.some(Number.isFinite)) refreshGuestStageLayout();
    perch.classList.remove("pose-sleep", "pose-wave", "pose-groom", "serving-drink");
    perch.classList.add("walking");
    perch.dataset.direction = "right";
  }

  function animatePpomiServe() {
    const perch = $("#ppomiPerch");
    if (!perch) return;
    perch.classList.add("serving-drink");
    setTimeout(() => perch.classList.remove("serving-drink"), 760);
  }

  function tickPpomi(elapsed) {
    const perch = $("#ppomiPerch");
    if (!perch?.classList.contains("walking")) return;
    if (!State.ppomi.centers.some(Number.isFinite)) refreshGuestStageLayout();
    const leftBound = State.ppomi.leftBound;
    const rightBound = State.ppomi.rightBound;
    if (!(rightBound > leftBound)) return;
    if (!State.ppomi.ready) {
      State.ppomi.x = leftBound;
      State.ppomi.direction = 1;
      State.ppomi.ready = true;
    }
    const previousX = State.ppomi.x;
    const movementDirection = State.ppomi.direction;
    let nextX = previousX + movementDirection * elapsed * .052;
    if (nextX >= rightBound) {
      nextX = rightBound;
      State.ppomi.direction = -1;
    ...
    State.ppomi.x = nextX;
    const ppomiLeft = `${nextX - State.ppomi.spriteWidth / 2}px`;
    perch.style.setProperty("--ppomi-x", ppomiLeft);
    perch.dataset.direction = State.ppomi.direction > 0 ? "right" : "left";

    for (const guest of Guests.slice(0, guestCapacityForLevel())) {
      if (!guest.active || guest.serving || guest.eating) continue;
      const drink = pendingItems(guest).find(item => MenuCatalog[item.id]?.kind === "drink");
      if (!drink || ingredientStock(drink.id) <= 0) continue;
      const targetX = State.ppomi.centers[guest.index];
      if (!Number.isFinite(targetX)) continue;
      const crossed = movementDirection > 0
        ? previousX <= targetX && nextX >= targetX
        : previousX >= targetX && nextX <= targetX;
      const guestKey = `${guest.index}:${guest.visitSerial}:${drink.id}`;
      if (!crossed || State.ppomi.lastGuestKey === guestKey) continue;
      State.ppomi.lastGuestKey = guestKey;
      deliverOrderItem(guest.index, drink.id, null, { servedBy: "ppomi" });
      break;
    }
  }
  ...
  function teleport(appliance, text) {
    ...
    const pose = appliance.type === "pot" ? (IngredientRules[appliance.item]?.mode === "addon" ? (appliance.item === "egg" ? "egg" : "tongs") : "noodle") : appliance.type;
  ...
  function isBatchAppliance(appliance) {
    return appliance?.type === "oden" || (appliance?.type === "grill" && appliance.recipeId === "grilled_dumpling");
  }

  function batchIngredientFor(appliance) {
    return appliance?.type === "oden" ? "oden" : "dumpling";
  }
  ...
  function startCooking(appliance, item) {
    ...
    appliance.servingsShown = isBatchAppliance(appliance) ? 3 : 0;
    const recipe = recipeFor(appliance);
  ...
  function takeApplianceServing(appliance) {
    if (!isBatchAppliance(appliance)) {
      resetAppliance(appliance);
      return;
    }
    const ingredientId = batchIngredientFor(appliance);
    const label = IngredientCatalog[ingredientId].label;
    const servings = appliance.servingsShown || 3;
    if (servings <= 1) {
      if (!consumeIngredient(ingredientId)) {
        resetAppliance(appliance);
        toast(`${label} 재료가 없어 조리도구가 비었어요. 재료 상점에서 보충해 주세요.`);
        return;
      }
      appliance.servingsShown = 3;
      toast(`${appliance.type === "oden" ? "오뎅바" : "만두 그릴"}에 3개를 다시 채웠어요. ${label} 재료 1개 사용`);
    } else {
  ...
  function completeOrder(guest) {
    ...
    guest.serving = true;
    guest.eating = true;
    guest.eatingRemaining = Config.guests.eatingMs;
    guest.eatingKind = eatingKindForGuest(guest);
    ...
    toast(storyMoment ? `새 이야기: ${storyMoment.title}` : `주문 완료 +${money(price)} · 10초 동안 천천히 식사해요`);
  }

  function deliverOrderItem(guestIndex, itemId, appliance = null, options = {}) {
    ...
    orderItem.fulfilled = true;
    if (appliance) takeApplianceServing(appliance);
    if (options.servedBy === "ppomi") animatePpomiServe();
    else teleportToGuest(guestIndex);
    renderGuest(guest);
    Sound.sfx("serve");
    Sound.haptic(16);
    ...
    if (remaining) {
      say(options.servedBy === "ppomi" ? `뽀미가 ${MenuCatalog[itemId].label}을 전했어!` : `${MenuCatalog[itemId].label} 먼저 드릴게요!`);
      toast(options.servedBy === "ppomi" ? `뽀미 자동 서빙 · ${MenuCatalog[itemId].label} · 음식 1개 남았어요.` : `${MenuCatalog[itemId].label} 전달 · ${remaining}개 남았어요.`);
    } else {
      completeOrder(guest);
    }
    return true;
  ...
  function nextProgressionText() {
    ...
    return next ? `${next.label}: 좌석 ${next.seats}석 · 손님 ${next.customers}명` : "최대 좌석 5석 · 손님 18명 해금 완료";
  }
  ...
  async function browserQA() {
    ...
    let patchV0266CssSource = "";
    let patchV0267CssSource = "";
    let patchV0269CssSource = "";
    try {
      const [manifestResponse, cssResponse, experienceResponse, mobileResponse, storyResponse, bootResponse, workerResponse, patchResponse, patchV0262Response, patchV0263Response, patchV0264Response, patchV0265Response, patchV0266Response, patchV0267Response, patchV0269Response] = await Promise.all([
        ...
        fetch("boot-v0269.js", { cache: "no-store" }),
        ...
        fetch("patch-v0266.css", { cache: "no-store" }),
        fetch("patch-v0267.css", { cache: "no-store" }),
        fetch("patch-v0269.css", { cache: "no-store" })
      ]);
      ...
      patchV0266CssSource = await patchV0266Response.text();
      patchV0267CssSource = await patchV0267Response.text();
      patchV0269CssSource = await patchV0269Response.text();
    } catch {
    ...
    result.parallelCriticalLoading = bootSource.includes("preloadCriticalAssets")
      ...
      && bootSource.includes('version: "0.26.9"');
    ...
    result.saveRecoveryReady = !!$("#exportSaveButton")
      ...
      && exportProbe.gameVersion === "0.26.7";
    ...
    result.hudReadability = hudRect.height >= 100 * buttonStageScale
      ...
      && dayCell.querySelector("small").textContent.trim() === "포차 레벨"
      && dayCell.querySelector("b").textContent.trim() === "LV.1"
      && dayCell.querySelector("em").textContent.replace(/\s+/g, " ").trim() === "DAY 1";
    ...
    result.completeHudOrder = $$(".hud>div>small").map(label => label.textContent.trim()).join("|")
      === "포차 레벨|총 누적 금액|보유 금액|남은 시간|현재 매출|오늘 손님"
      && !$("#walletBadge")
      && $("#walletGold").closest(".hud") === $(".hud");
    const signRect = $(".sign").getBoundingClientRect();
    const utilityRects = ["#quickSupplyButton", "#recipeButton", "#journalButton", "#helpButton", "#soundButton"]
      .map(selector => $(selector).getBoundingClientRect());
    result.unifiedTopFrame = signRect.left <= 20 * buttonStageScale
      && Math.abs(signRect.top - hudRect.top) <= 3 * buttonStageScale
      && hudRect.left >= signRect.left + signRect.width - 18 * buttonStageScale
      && Math.abs(utilityRects[0].top - hudRect.top) <= 3 * buttonStageScale
      && utilityRects.every((rect, index) => !index || Math.abs(rect.top - utilityRects[index - 1].bottom - 8 * buttonStageScale) <= 2 * buttonStageScale);
    ...
    result.guestCounterReachesBothEdges = Math.abs(guestCounterRect.left - guestStageRect.left) <= 2
      ...
      && Math.abs(parseFloat(guestCounterStyle.top) + parseFloat(guestCounterStyle.height) - 644) <= .5;
    result.guestGroupLoweredTogether = parseFloat(getComputedStyle($("#guestRow")).top) === 172
      && parseFloat(getComputedStyle($("#ppomiPerch")).top) === 294
      && parseFloat(guestCounterStyle.top) === 454;
    result.mobileDialogueUsesLogicalGuestCoordinates = patchV0266CssSource.includes("large enough to read")
      && showNextGuestDialogue.toString().includes("guestRow.offsetLeft")
      && showNextGuestDialogue.toString().includes("artMatrix")
      && patchV0267CssSource.includes("--whisper-tip-x")
      && parseFloat(getComputedStyle($("#closeStoryWhisperButton")).width) >= (document.documentElement.dataset.mobileLayout === "true" ? 46 : 34);
    result.dialogueRaisedTowardGuest = showNextGuestDialogue.toString().includes("guestBox.height - 94")
      && showNextGuestDialogue.toString().includes('"--whisper-y", "352px"');
    result.mobileViewportFillReady = patchV0266CssSource.includes("every pixel offered")
      && window.BoreumiPWA?.logicalViewport?.width >= Math.max(innerWidth, innerHeight);
    ...
    result.referenceStyleItemLabels = $$(".appliance label").length === 0
      && dockItemNames.length === 7
      && ["면", "계란", "대파", "김치", "치즈", "군만두", "오뎅"].every(name => dockItemNames.includes(name))
      && $$(".drink-stock").length === 4;
    result.noDailyMaterialsCell = !$(".dock-label");
    result.sharedDisplayRacks = $$(".ingredient-rack .ingredient").length >= 2
      && !$(".drink-rack")
      && $$(".drink-stock-rail .drink-stock").length === 4
    ...
    result.referenceStyleDock = $$(".rack-title").map(title => title.textContent.trim()).join("|") === "라면 재료|안주"
      && $$(".inventory-rack").every(rack => getComputedStyle(rack).borderImageSource.includes("dock-rack-frame-v1.webp"));
    ...
    result.extensibleInventory = InventoryCategories.length === 3
      && Config.layout.inventoryPageSize === 5
      && Config.layout.inventoryCategories.join("|") === "ramen|drinks|anju"
      && $$(".inventory-rack").length === 2
      && $$(".inventory-rack").every(rack => rack.dataset.pageSize === "5" && rack.querySelectorAll(".rack-page").length === 2)
      && Number(getComputedStyle($(".ingredient-rack")).flexGrow) === 5
      && Number(getComputedStyle($(".snack-rack")).flexGrow) === 2;
    result.inventoryPaginationFlow = $$(".ingredient-rack .catalog-item").length === 5
      && $$(".snack-rack .catalog-item").length === 2
      && $$(".dock .rack-page,.dock .rack-page-index").every(control => getComputedStyle(control).display === "none");
    const drinkArt = {
    ...
    result.pauseJournalReady = $("#pauseOverlay").getAttribute("role") === "dialog"
      && $("#pauseJournalTitle").textContent.includes("영업 중")
      && $("#pauseJournalCumulative").textContent === money(cumulativeSales())
      && $("#pauseJournalSales").textContent === money(State.sales)
      && $("#resumeButton").textContent.trim() === "계속 하기";
    $("#resumeButton").click();
    ...
    result.drinkPricesRaised = MenuCatalog.soju.price === 2500
      && MenuCatalog.beer.price === 3000
      && MenuCatalog.somaek.price === 3500
      && MenuCatalog.makgeolli.price === 3000;
    result.drinksAreDraggable = !$(".drink-rack")
      && $$(".drink-stock").length === 4
      && $$(".drink-stock").every(item => item.querySelector("img") && item.querySelector("b"));
    result.calmerOrderRhythm = Config.guests.eatingMs === 10000
      && Config.guests.nextArrivalMs === 5000
      && Config.takeout.repeatDelayMs === 30000;
    result.ppomiAutomaticDrinkService = tickPpomi.toString().includes('servedBy: "ppomi"')
      && startPpomi.toString().includes("walking")
      && $$(".drink-stock").length === drinkOrderPool().length;
    result.ppomiNaturalWalkFrames = patchV0269CssSource.includes("ppomi-walk-strip-v1.webp")
      && patchV0269CssSource.includes("ppomiNaturalWalk")
      && patchV0269CssSource.includes("background-position:100% 50%")
      && tickPpomi.toString().includes("State.ppomi.centers")
      && !tickPpomi.toString().includes("stageBoxFor(slot)");
    result.busyScenePerformanceBudget = burstAt.toString().includes("particleCount")
      && burstAt.toString().includes("childElementCount")
      && tickGuests.toString().includes("guest.eatingRemaining")
      && !tickGuests.toString().includes("previousSecond")
      && managedTimer.toString().includes("collection.splice")
      && patchV0269CssSource.includes("translate3d(var(--ppomi-x");
    ...
    const mobileControls = [$("#quickSupplyButton"), $("#recipeButton"), $("#journalButton"), $("#helpButton"), $("#soundButton")];
    ...
    result.ramenMoonSign = signStyle.backgroundImage.includes("sign-ramen-moon-v1.webp")
      && parseFloat(signStyle.borderTopWidth) === 0
      && $(".sign").textContent.replace(/\s+/g, "").includes("보름이의라면포차")
      && bootSource.includes("sign-ramen-moon-v1.webp");
    ...
    result.naturalCookingToppingArt = getComputedStyle(toppingSpriteProbe).backgroundImage.includes("food-ramen-kimchi-red-v1.webp")
      && getComputedStyle(toppingSpriteProbe, "::after").display === "none";
    result.redKimchiVariants = FoodArt.potKimchi.includes("kimchi-red-v1.webp")
      && FoodArt.potKimchiEgg.includes("kimchi-egg-red-v1.webp")
      && FoodArt.potKimchiCheese.includes("kimchi-cheese-red-v1.webp")
      && patchV0269CssSource.includes("Richer chili red");
    ...
    result.naturalComboCookingArt = getComputedStyle(comboSpriteProbe).backgroundImage.includes("food-ramen-kimchi-cheese-red-v1.webp")
    ...
    Appliances[1].item = "scallion";
    teleport(Appliances[1], "집게 동작 검사");
    result.nonEggToppingUsesTongs = $("#boreumi").dataset.pose === "tongs"
      && getComputedStyle($("#boreumi")).backgroundImage.includes("boreumi-tongs-v1.webp");
    Appliances[1].item = "egg";
    ...
    result.completeFoodArt = !!$(".sprite-cooking-dumpling")
      && getComputedStyle($(".sprite-cooking-dumpling")).backgroundImage.includes("cooking-dumpling.webp");
    ...
    takeApplianceServing(Appliances[3]);
    const dumplingTwoArt = getComputedStyle($(".sprite-cooking-dumpling-two"), "::after").backgroundImage;
    takeApplianceServing(Appliances[3]);
    const dumplingOneArt = getComputedStyle($(".sprite-cooking-dumpling-one"), "::after").backgroundImage;
    const dumplingStockBeforeRefill = ingredientStock("dumpling");
    takeApplianceServing(Appliances[3]);
    result.dumplingAutoRefills = Appliances[3].state === "ready"
      && Appliances[3].recipeId === "grilled_dumpling"
      && Appliances[3].servingsShown === 3
      && ingredientStock("dumpling") === dumplingStockBeforeRefill - 1
      && dumplingTwoArt.includes("cooking-dumpling.webp")
      && dumplingOneArt.includes("cooking-dumpling.webp")
      && $("[data-id='grill-0']").dataset.servingCount === "3";
    ...
    const sojuStock = $(`[data-drink-stock="soju"]`);
    result.drinkGhostIllustration = !$(".drink-rack")
      && Boolean(sojuStock?.querySelector("img")?.src)
      && sojuStock?.querySelector("b")?.textContent.endsWith("개");
    if (Guests[0].order.items.some(item => item.id === "soju" && !item.fulfilled)) {
      deliverOrderItem(0, "soju", null, { servedBy: "ppomi" });
    }
    ...
    if (qaParams.has("previewEating")) await new Promise(resolve => setTimeout(resolve, 4500));
    await new Promise(resolve => setTimeout(resolve, 850));
    result.guestLeavesAfterServe = Guests[0].active
      && Guests[0].eating
      && Guests[0].eatingRemaining < Config.guests.eatingMs
      && Guests[0].eatingRemaining > (qaParams.has("previewEating") ? 3500 : Config.guests.eatingMs - 1500)
      && $(`[data-guest="0"]`).classList.contains("eating")
      && $(`[data-guest-meal="0"]`).hidden === false
      && $$(`[data-guest-meal="0"] .guest-meal-item`).length === Guests[0].order.items.length
      && !$(`[data-guest="0"] .eating-scene`);
    result.mealHasNoVisibleTimer = !$(`[data-guest-meal="0"]`).textContent.trim()
      && !patchV0269CssSource.includes("guestEatingBite");
    const guestTimerCountBeforeEatingEnd = State.guestTimers.length;
    Guests[0].eatingRemaining = 60;
    await new Promise(resolve => setTimeout(resolve, 180));
    result.eatingThenFiveSecondTurnover = !Guests[0].active
      && !Guests[0].eating
      && State.guestTimers.length > guestTimerCountBeforeEatingEnd
      && Config.guests.nextArrivalMs === 5000;
    ...
    const walletRect = $("#walletGold").closest("div").getBoundingClientRect();
    result.walletHudReadable = walletRect.left >= hudRect.left
      && walletRect.right <= hudRect.right
      && $("#walletGold").textContent === money(Progress.gold)
      && $("#stallLevel").textContent === String(effectiveStallLevel())
      && !$("#walletBadge");
    result.managementUpgradeCatalog = Object.keys(StationUpgradeCatalog).join("|") === "pot|grill|oden"
      ...
      && StationUpgradeCatalog.pot.costs[0] === 18000
      && StationUpgradeCatalog.grill.costs[0] === 16000
      && StationUpgradeCatalog.oden.costs[0] === 14000
      && StallUpgradeCatalog.costs[0] === 90000;
    ...
    const milestoneCases = [
      ...
      { day: 50, level: 2, seats: 5, customers: 10, width: 1920 },
      { day: 1, level: 3, seats: 5, customers: 12, width: 1920 },
      { day: 1, level: 4, seats: 5, customers: 15, width: 1920 },
      { day: 1, level: 5, seats: 5, customers: 18, width: 1920 }
    ];
    ...
    resetGuests();
    State.running = true;
    for (let index = 0; index < 5; index += 1) activateGuest(index);
    const maxSeatCustomerIds = Guests.filter(guest => guest.active).map(guest => guest.customerId);
    result.tenSeatRandomVisitors = maxSeatCustomerIds.length === 5
      && new Set(maxSeatCustomerIds).size === 5
    ...
    result.tenSeatLayoutNoOverlap = maxSeatRects.length === 5
    ...
    const maxGuestArtRects = $$(".guest-slot:not([hidden]) .guest-art").map(art => art.getBoundingClientRect());
    const expansionStageRect = $("#stage").getBoundingClientRect();
    result.expansionFacilitiesUseSideWing = !$("#takeoutBoard").hidden
      && !$("#completionPass").hidden
      && boardRect.left >= expansionStageRect.left - 1
      && boardRect.right < passRect.left
      && passRect.right <= expansionStageRect.right + 1
      && passRect.top < maxDockRect.top
      && maxGuestArtRects.length === 5;
    result.largeTakeoutTickets = parseFloat(getComputedStyle($("#takeoutBoard")).width) >= 284
      && parseFloat(getComputedStyle($(".takeout-order")).height) >= 78
      && parseFloat(getComputedStyle($(".takeout-order .package-preview")).width) >= 48
      && parseFloat(getComputedStyle($(".takeout-board>header strong")).fontSize) >= 22;
    ...
    result.completionPassStoresFood = Appliances[3].state === "ready"
      && Appliances[3].servingsShown === 2
  ...
  function renderPauseJournal() {
    $("#pauseJournalDay").textContent = String(effectiveDay());
    $("#pauseJournalCumulative").textContent = money(cumulativeSales());
    $("#pauseJournalSales").textContent = money(State.sales);
    $("#pauseJournalGold").textContent = money(Progress.gold);
    $("#pauseJournalGuests").textContent = `${State.guests}명`;
    $("#pauseJournalServed").textContent = `${State.served}건`;
    $("#pauseJournalTakeout").textContent = `${State.takeoutServed}건`;
  }

  function openPauseJournal() {
  ...
  function resumeFromPauseJournal() {
    if (!State.running) return;
    State.paused = false;
    $("#stage").classList.remove("paused-fx");
  ...
  $("#pauseButton").addEventListener("click", openPauseJournal);
  $("#resumeButton").addEventListener("click", resumeFromPauseJournal);
